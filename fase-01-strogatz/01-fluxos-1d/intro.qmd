...
:::: {.chapter-header-card}
[Semana 1 · Fase 1 — Strogatz]{.chapter-week}

Tempo estimado de leitura: **60–90 min**.

[Dependências: nenhuma]{.chapter-meta}
::::
...
![Bola descendo o gradiente de $V(N)$ até o mínimo local em $N=K$. A trajetória é monótona; o ponto fixo estável é literalmente o fundo da bacia.](../../assets/figs/fig-bacia-potencial.svg){#fig-bacia-potencial fig-alt="Gráfico do potencial V(N) para o modelo logístico com bola simbólica em quatro posições sucessivas, descendo até o mínimo local em N igual a K"}
...
![Carregamento RC: $V(t)$ aproxima-se exponencialmente de $V_0$ com tempo característico $\tau = RC$. Em $t = \tau$, o sistema cobriu 63% do caminho.](../../assets/figs/fig-rc-charging.svg){#fig-rc-charging fig-alt="Curva exponencial crescente saturando em V0, com marcação de tempo característico tau no eixo horizontal e valor 0.632 V0 no eixo vertical"}
...
![Retrato de fase de $\dot{x} = x - x^3$. Pontos fixos estáveis em $x = \pm 1$ (círculos cheios), instável em $x = 0$ (círculo vazio). As setas seguem o sinal de $f$ e convergem aos estáveis; cada um tem sua bacia de atração separada por $x = 0$.](../../assets/figs/fig-retrato-fase-1d.svg){#fig-retrato-fase-1d fig-alt="Eixo horizontal com três pontos fixos marcados: x igual a menos um, zero e mais um, com setas convergindo aos extremos e divergindo do central"}
...
5. Calcule o potencial $V(x)$ e confirme que pontos fixos estáveis são mínimos locais; descreva a forma do gráfico de $V$.
6. Em linguagem cibernética: qual o "regulador implícito" e qual sua "variedade requisita"?

Para fixar: quando terminar, importe o deck Anki (vide [revisão](../../revisao/index.qmd)) — os cartões `strogatz fase1` cobrem precisamente os conceitos deste capítulo.

::: {.callout-tip}
Se você está estudando Joana como caso ([Personagem](../../personagem-narrativo.qmd)): proponha um indicador acadêmico real de polo UAB-UNITINS ou disciplina UNIFAL-MG cujo comportamento agregado se aproxime de um sistema 1D autônomo. Estime visualmente os pontos fixos a partir de séries históricas e classifique-os.
:::
