...
<div class="painel-opsroom">
  <div class="botao ativo"><span class="label">Semana</span><span class="valor">18 de 24</span></div>
  <div class="botao"><span class="label">Fase</span><span class="valor">Síntese · âncora</span></div>
  <div class="botao"><span class="label">Tempo</span><span class="valor">90–120 min</span></div>
</div>
```

::: {.cena}
[<img src="../../assets/figs/personas/juliana.svg" class="persona-icone"/>]{.persona-tag .juliana}**Juliana, sexta-feira, mesa de reunião do GAN, Cristalândia-TO.**

Sete pessoas em torno de uma mesa: dois fiscais da Naturatins, um promotor do MPTO, dois engenheiros agrônomos representando rizicultores, um técnico da ANA via videoconferência, ela. Pauta única: a outorga conjunta da safra 2026 dado o regime de chuvas projetado.

Em quatro horas de reunião, observou três falhas estruturais — a Naturatins não tinha dado em tempo real, o MPTO operava por ação tardia, os rizicultores planejavam isoladamente. *Cada um era ótimo no seu pedaço. O sistema, como sistema, não estava em lugar nenhum.*

No caderno escreveu: "S2 ausente. S3 fraco. S4 inexistente. Esta reunião é embrião — pode virar S2 estável se durar." Saiu da sala vinte minutos tarde para o almoço, sabendo que tinha capítulo de tese.
:::

::: {.simples}
Imagine uma orquestra sem maestro. Cada músico é ótimo. Cada instrumento toca certo. Mas ninguém escuta o outro, ninguém marca compasso, ninguém decide *qual* música tocar nem *quando* parar.

Isso pode funcionar por algum tempo se todos vierem de uma mesma escola. Mas quando muda o repertório, quando entra músico novo, quando o palco ficar diferente — o som vira ruído.

**O Modelo do Sistema Viável de Stafford Beer é a partitura do maestro**: cinco funções que, juntas, transformam um conjunto de boas execuções num sistema que aprende, se adapta, sobrevive. As funções têm nomes engraçados (S1, S2, S3, S4, S5), mas o que cada uma faz é direto: tocar, coordenar, alocar, prever, decidir o que somos.
:::
...
Este capítulo articula três pontos: (i) o que cada fase técnica anterior contribui para a linguagem do VSM; (ii) como a obra de Juliana Mariano Alves traduz essa linguagem para a governança hídrica brasileira (BHRF — Bacia Hidrográfica do Rio Formoso, Tocantins); (iii) o que isso implica para a **pesquisa-cenário do curso** — a comparação ficcional, conduzida por Joana Beraldo, entre a UNITINS (Palmas-TO, EaD via UAB) e a UNIFAL-MG (Alfenas, presencial) como dois sistemas viáveis com a mesma função-fim e arquiteturas opostas (vide [Personagem](../../personagem-narrativo.qmd)).

```{=html}
<div class="cards-conceito">
  <div class="card-conceito">
    <h4>S1 Operação</h4>
    <p>Cada divisão autônoma que produz a saída-fim. Outorga, política finalística, polo UAB.</p>
  </div>
  <div class="card-conceito">
    <h4>S2 Antioscilação</h4>
    <p>Coordenação lateral entre S1s. Impede oscilação por uso conflitante. <em>Clearing house</em>.</p>
  </div>
  <div class="card-conceito">
    <h4>S3 + S3* Aqui-agora</h4>
    <p>Aloca recursos entre S1s, audita. <span class="formula">S3*</span> = canal esporádico de verificação.</p>
  </div>
  <div class="card-conceito">
    <h4>S4 Fora-e-depois</h4>
    <p>Inteligência: ambiente externo, futuro, adaptação.</p>
  </div>
  <div class="card-conceito">
    <h4>S5 Política/identidade</h4>
    <p>Define o que o sistema é e o que se recusa a ser.</p>
  </div>
  <div class="card-conceito">
    <h4>Sinal algedônico</h4>
    <p>Canal vertical baixíssima banda S1 → S5 em emergência. Contorna S2/S3/S4.</p>
  </div>
</div>
```
...
![Os cinco subsistemas do VSM e os canais que os conectam. S1 são unidades operacionais autônomas; S2 (lateral, ocre) coordena entre S1s; S3 aloca aqui-agora, com S3* esporadicamente auditando; S4 monitora fora-e-depois; S5 política/identidade. O canal algedônico (vermelho) liga S1 diretamente a S5 em emergência.](../../assets/figs/canonicas/fig-vsm-5-sistemas.svg){#fig-vsm-5-sistemas fig-alt="Diagrama do VSM com três círculos S1 à esquerda, S2 lateral entre eles em ocre, S3 e S4 e S5 empilhados à direita, S3 estrela auditoria, e seta vermelha algedônica do S1 superior ao S5"}
...
A pesquisa de Joana Beraldo (vide [Personagem](../../personagem-narrativo.qmd)) toma duas universidades públicas brasileiras como sistemas viáveis comparáveis — domínio em que o VSM é particularmente útil porque cada universidade é, ela própria, um sistema viável recursivo (curso ⊂ instituto ⊂ universidade ⊂ rede federal/estadual). Três caminhos imediatos:
...
![Diagrama de Conant-Ashby. Distúrbios $D$ atingem o sistema $S$ via $g$ e o regulador $R$ via $f$; $R$ atua sobre $S$; o resultado essencial $E$ depende de ambos. O teorema afirma que $f$ é homomorfa a $g$ quando $R$ minimiza a entropia residual em $E$.](../../assets/figs/canonicas/fig-conant-ashby.svg){#fig-conant-ashby fig-alt="Quatro elipses representando Distúrbios D, Sistema S, Regulador R e Essencial E, com setas g de D para S, f de D para R, e setas de R para S e de S e R para E"}
...
<div class="dialogo">
  <div class="fala juliana">
    ...
    <div class="o-que">O VSM dá nome ao que eu via desde 2018 no GAN sem conseguir nomear. Não é "falta de governança" no abstrato — é S2 estruturalmente ausente. O nome muda a intervenção: você não pede "mais governança", pede um cargo concreto cujo trabalho é coordenar lateralmente.</div>
  </div>
  <div class="fala joana">
    <div class="quem">Joana</div>
    <div class="o-que">Aplicado ao polo UAB-UNITINS de Augustinópolis: cada turma é um S1, a coordenação local é S2/S3, a pró-reitoria é S3/S4, a UAB nacional é S4/S5. Diagnóstico: S2 inter-polos é o que mais falta. Cada coordenação local descobre os problemas das outras tarde demais.</div>
  </div>
  <div class="fala luiz">
    <div class="quem">Luiz Eduardo</div>
    <div class="o-que">Em PDI a hierarquia é a mesma: filtragem (baixo) → segmentação (médio) → interpretação (alto). Cada nível é S1 do nível superior. Um pixel mal segmentado é diagnóstico de S2 ausente entre filtros. Uma interpretação errada é S5 confuso. Beer escreveu o aparato genérico — eu só uso o vocabulário em outro domínio.</div>
  </div>
</div>
```
...
<div class="proximo-passo">
  ...
  <strong>Semana 19 — Bacia do Formoso: caso BHRF</strong><br>
  <span style="opacity:0.8">A teoria do VSM aplicada num conflito real: outorgas, MPTO 2016, GAN, recursão tripla.</span><br>
  <a href="../02-bacia-formoso-caso/intro.html">Continuar →</a>
</div>
```
